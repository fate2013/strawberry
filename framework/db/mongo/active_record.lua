local Query = require "framework.db.mongo.query"
local Schema = require "framework.db.mongo.schema"
local Replica = require "framework.db.mongo.replica"
...
function ActiveRecord:get_replica()
    ...
    replica = Replica:instance(self.config_group, Registry.app.config.mongo[self.config_group])
...
local function insert(self)
    return Query:new(self):insert(self.collection_name, self.attributes)
end
...
function ActiveRecord:save()
    if self.is_new then
        local affected_rows, err = insert(self)
        local success = affected_rows > 0
        if success then
            self.is_new = false
        end
        return success
